...
export function displayUploadModal(cropAspect, uploadProps) {
    uploadModal.style.display = "block";
    // Store the original aspect ratio
    cropAspectRatio = cropAspect;
    console.log("Setting crop aspect ratio:", cropAspectRatio);
    
    // If there's an existing cropper, update its aspect ratio
    if (cropper) {
        cropper.destroy();
    }
...
export function initUploadModal() {
    ...
    const cropCallback = () => {
        ...
        // Get the container data to calculate proper dimensions
        const containerData = cropper.getContainerData();
        const cropBoxData = cropper.getCropBoxData();
        
        // Calculate the output size while maintaining aspect ratio
    ...
    if (file && (file.type === 'image/png' || file.type === 'image/jpeg')) {
        ...
        reader.onload = function (e) {
            ...
            cropper = new Cropper(cropPreview, {
                ...
                movable: true,
                ready: function() {
                    // Set initial crop box to match aspect ratio
                    const containerData = cropper.getContainerData();
                    const cropBoxWidth = Math.min(containerData.width, containerData.height * cropAspectRatio);
                    const cropBoxHeight = cropBoxWidth / cropAspectRatio;
