...
export function addAgent(position, agentParams = {}, userData = {}) {
 ...
 const params = {
   radius: agentParams.radius ?? 0.1,
   height: agentParams.height ?? 2.0,
   maxAcceleration: agentParams.maxAcceleration ?? 8.0,
   maxSpeed: agentParams.maxSpeed ?? 3.5,
   collisionQueryRange: agentParams.collisionQueryRange ?? 0.0,
   pathOptimizationRange: agentParams.pathOptimizationRange ?? 10.0,
   separationWeight: agentParams.separationWeight ?? 0.0,
 };
...
export function startAgentTour(agentEntry, PictureMeshes = [], navQuery, options = {}) {
   ...
   const targetsMap = options.targetsDestinationMap || new Map();
   const queue = [];
...
// FUNCTION TO ADD THIRD PERSON TO CROWD , IN ANOTHER WORD , WE WILL INIT THE THIRD PERSON PLAYER AS AN AGENT THEN ADD IT TO THE CROWD
// FUNCTION TO ADD THIRD PERSON TO CROWD
export function addThirdPersonToCrowd(scene, crowd, tpView) {
 ...
 // Use addAgent (the function you already have), not addAgentToCrowd
 const agent = addAgent(pos, {
   radius: 0.05, // A very small radius to allow close proximity
   height: 2.0,
   maxSpeed: 3.0,
   maxAcceleration: 6.0,
   collisionQueryRange: 0.0, // This is key: agent will ignore collisions
   separationWeight: 0.0, // This is also key: agent will not push away from others
 ...
 if (!agent) {
  ...
  // Link the agent to the ThirdPersonPlayer object
 tpView.crowdAgent = agent;
 return agent;
}
