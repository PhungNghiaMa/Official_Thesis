...
export default class ThirdPersonPlayer {
    ...
    loader.load(url, (gltf) => {
      ...
      if (gltf.animations && gltf.animations.length > 0) {
        gltf.animations.forEach((clip) =>{
          switch(clip.name){
            case "WalkForward":
              this.walkForwardAction = this.mixer.clipAction(clip);
              break;
            case "TurnLeft":
              this.turnLeftAction = this.mixer.clipAction(clip);
              break
            case "TurnRight":
              this.turnRightAction = this.mixer.clipAction(clip);
              break;
            case "RunForward":
              this.runForwardAction = this.mixer.clipAction(clip);
              break;
            default:
              break;    
          }
  ...
  update(delta, cameraYaw) {
    ...
    if (this.input.forward){
      if (this.input.run && this.runForwardAction){
        this.playAction(this.runForwardAction);
      }else if (this.walkForwardAction){
        this.playAction(this.walkForwardAction);
      }
    }else if (this.input.left && this.turnLeftAction){
      this.playAction(this.turnLeftAction);
    }else if (this.input.right && this.turnRightAction){
      this.playAction(this.turnRightAction);
    }else if (this.input.backward){
      if(this.walkForwardAction){
        this.action.playAction(this.walkForwardAction);
      }
