...
// fixPlaylist rewrites segment paths in a variant playlist to include the stream folder prefix.
func fixPlaylist(path string, streamName string) error {
    ...
    for i, line := range lines {
        if strings.HasSuffix(line, ".ts") && !strings.Contains(line, "/") {
            // Prepend the stream folder name
            lines[i] = streamName + "/" + line
        }
    }
