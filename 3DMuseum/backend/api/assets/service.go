...
func (s *AssetService) UploadAsset(ctx context.Context, info model.DetailUploadInfor) (*AssetUploadResult, error) {
	...
	reportProgress := func(stage, status string, progress int, message string, extra map[string]interface{}) {
		...
		if assetCID != "" {
			websocket.GlobalHub.BroadcastProgress("asset:"+assetCID, msg)
		}
	}
	...
	if !isVideoUpload{
		...
		VideoConversionResult, err := business.ConvertToHLS(ctx , tempPath)
		if err != nil {
		...
		uploadVideoResponse , err := s.PinataRepo.UploadVideoToPinata(VideoConversionResult.FolderPath , roomChannel , "");
		if err != nil {
	...
	for _, j := range jobs {
		...
		go func(j job) {
			...
			progressMsg := func(p int, msg string) {
				...
				reportProgressAudio(assetChannel, j.Lang, "processing", msg, p, extra)
				reportProgressAudio(roomChannel, j.Lang, "processing", msg, p, extra)
			}
			...
			if err != nil {
				...
				reportProgressAudio(assetChannel, j.Lang, "failed", fmt.Sprintf("Failed to generate %s TTS audio.", j.Lang), 0, extra)
				reportProgressAudio(roomChannel, j.Lang, "failed", fmt.Sprintf("Failed to generate %s TTS audio.", j.Lang), 0, extra)
				return
			}
			...
			resp, err := s.PinataRepo.UploadAudioToPinata(audioData, fileName, assetChannel, assetCID, j.Lang)
			if err != nil {
				attempts++
				_ = s.AssetRepo.UpdateAudio(ctx, assetCID, j.Lang, "failed", "", attempts)
				extra := map[string]interface{}{"asset_cid": assetCID, "error": err.Error()}
				reportProgressAudio(assetChannel, j.Lang, "failed", fmt.Sprintf("Failed to upload %s TTS audio.", j.Lang), 0, extra)
				reportProgressAudio(roomChannel, j.Lang, "failed", fmt.Sprintf("Failed to upload %s TTS audio.", j.Lang), 0, extra)
				return
			}
			...
			reportProgressAudio(assetChannel, j.Lang, "completed", fmt.Sprintf("%s TTS audio uploaded successfully.", j.Lang), 100, extra)
			reportProgressAudio(roomChannel, j.Lang, "completed", fmt.Sprintf("%s TTS audio uploaded successfully.", j.Lang), 100, extra)
		}(j)
	}
...
func reportProgressAudio(channel, lang, status, message string, progress int, extra map[string]interface{}) {
	msg := map[string]interface{}{
		"type":       "tts",
		"language":   lang,
		"status":     status,
	...
	wsWriteMutex.Lock()
	defer wsWriteMutex.Unlock()
	websocket.GlobalHub.BroadcastProgress(channel, msg)
}